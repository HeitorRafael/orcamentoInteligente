...
exports.createBudget = async (req, res) => {
    ...
    if (!client_name || !project_name || !user_id) {
        return res.status(400).json({ message: 'Nome do cliente, nome do projeto e ID do usuário são campos obrigatórios para um orçamento.' });
    }
    ...
    try {
        const budget = await Budget.create({
            ...
            total_price: total_price || 0.00, // Define 0 como preço total padrão
            notes,
        });
...
exports.updateBudget = async (req, res) => {
    ...
    try {
        ...
        // Atualiza os campos
        budget.client_name = client_name !== undefined ? client_name : budget.client_name;
        budget.project_name = project_name !== undefined ? project_name : budget.project_name;
        budget.status = status !== undefined ? status : budget.status;
        budget.total_price = total_price !== undefined ? total_price : budget.total_price;
