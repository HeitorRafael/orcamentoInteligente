...
exports.createInput = async (req, res, next) => {
  ...
  if (!product_service_id || !name || !quantity) {
    return res
      ...
      .json({
        message:
          "ID do produto/serviço, nome e quantidade são campos obrigatórios para um insumo.",
      });
  }
  ...
  try {
    ...
    if (!input) {
      return res
        .status(404)
        .json({
          message: "Insumo não encontrado ou não pertence a este usuário.",
        });
    }
...
exports.updateInput = async (req, res, next) => {
  ...
  try {
    ...
    input.description =
      description !== undefined ? description : input.description;
    input.quantity = quantity !== undefined ? quantity : input.quantity;
    input.unit = unit !== undefined ? unit : input.unit;
    input.cost_per_unit =
      cost_per_unit !== undefined ? cost_per_unit : input.cost_per_unit;
    input.supplier_suggestion =
      supplier_suggestion !== undefined
        ? supplier_suggestion
        : input.supplier_suggestion;
    input.supplier_link =
      supplier_link !== undefined ? supplier_link : input.supplier_link;
