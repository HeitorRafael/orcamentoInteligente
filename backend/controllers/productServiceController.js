...
exports.createProductService = async (req, res) => {
  ...
  if (!name || !type || !user_id) {
    return res
      .status(400)
      .json({ message: "Nome, tipo e ID do usuário são campos obrigatórios." });
  }
  ...
  try {
    ...
    if (existingProductService) {
      return res
        .status(400)
        .json({ message: "Você já tem um produto/serviço com este nome." });
    }
...
exports.updateProductService = async (req, res) => {
  ...
  try {
    ...
    productService.estimated_time_hours =
      estimated_time_hours || productService.estimated_time_hours;
